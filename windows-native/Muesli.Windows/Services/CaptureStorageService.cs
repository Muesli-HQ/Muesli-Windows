        ...
        CapturedAudio? system)
    {
        ...
        try
        {
            micPath = microphone is null
                ? null
                : AtomicCopy(microphone.TranscriptionPath, Path.Combine(meetingDirectory, "microphone.wav"));
            systemPath = system is null
                ? null
                : AtomicCopy(system.TranscriptionPath, Path.Combine(meetingDirectory, "system.wav"));
        ...
        catch
        {
            ...
            {
                CapturedAudio.TryDelete(micPath);
            }
            if (systemPath is not null)
            {
                CapturedAudio.TryDelete(systemPath);
            }
            throw;
        }
    }
